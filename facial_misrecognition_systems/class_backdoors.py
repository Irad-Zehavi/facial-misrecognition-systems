...
# %% auto 0
__all__ = ['BackdoorData', 'ShatteredClass', 'MergedClasses']

# %% ../nbs/class_backdoors.ipynb 4
from fastai.vision.all import *

class BackdoorData(object):
    def __new__(cls, ds: Datasets, class_=None, min_samples=10, splitter=RandomSplitter(.1)):
        if class_ is None:
            class_, subset = random.choice(cls.candidates(ds, min_samples))
        else:
    ...
    @classmethod
    def candidates(cls, dataset, min_samples):
        return [(c, ss) for c, ss in dataset.by_target.items() if len(ss) > min_samples]
    
    @classmethod
    def random_pair(cls, *args, **kwargs):
        while True:
            bd1, bd2 = cls(*args, class_=None, **kwargs), cls(*args, class_=None, **kwargs)
